...
class Button_Image(Widget):
    def __init__(self, screen,master_x,master_y, x,y,w,h, path_image,
        ...
        self.isclicked_pause = False
        self._onclick_pause = onclick
        self._onclick_param_pause = onclick_param
        self._text_pause = text
    ...
    def update_pause(self, lista_eventos):
        self.isclicked_pause = False
        if self.contador_click > FPS/2:
            ...
            for evento in lista_eventos:
                if evento.type == pygame.MOUSEBUTTONDOWN:
                    if self.slave_rect_collide.collidepoint(evento.pos):
                        if self._onclick_pause != None and self._onclick_param_pause!=None:
                            self._onclick_pause(self._onclick_param_pause)
                        self.isclicked_pause = True
                        self.contador_click = 0
        else:
            self.contador_click += 1
