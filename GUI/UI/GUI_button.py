...
#clicked = None
class Button(Widget):
    def __init__(self, screen,master_x,master_y, x,y,w,h,color_background,color_border, onclick, onclick_param, text=None, font=None, font_size=None, font_color=None):
        ...
        self._font = pygame.font.Font("GUI/Recursos/slkscr.ttf",font_size)
        ...
        for evento in lista_eventos:
            if evento.type == pygame.MOUSEBUTTONDOWN:
                if self.slave_rect_collide.collidepoint(evento.pos):
                    clicked = True
                    if self._text == "Iniciar Partida":
                        self._onclick("Iniciar Partida")
                    elif self._text == "Usuario":
                        self._onclick("Usuario")
                    elif self._text == "Escenario":
                        self._onclick("Escenario")

                    
                    
                    
                    #elif self._text == "Puntajes":
                        #self._onclick("Puntajes")
                    break
        if not clicked:
            self.isclicked = False
        self.draw()
